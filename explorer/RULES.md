...
| 3 | Archives | roster length · 5+2 | **Home** = official G1 voting seats only (`officialVoting === true`, always **7**). P11 / G2 / Seoul extras are **not** Home seats. `/archives` lists **all** lab archive IPs (G1 + G2 + extras), grouped. |
...
## Lab new-chain page (`/lab-newchain`, 2026-08)

Detail page (footer hidden). **Not** Mock-L1 auction. Connect via **EIP-6963 installed-wallet picker** (same pattern as `https://beamio.app/usdc-topup`; ethers `BrowserProvider` on the **chosen** provider; no wagmi / WalletConnect). Wallet address fills Mode A `user`. Primary form classes: **Asset** (`classId=1`) and **Storage** (`classId=2`); Trade (`classId=3`) optional.

| Rule | Detail |
|---|---|
| Wire | Mirror `DleLabNewChainRequestV1` in `explorer/src/lib/labNewchainWire.ts` (aligned with `runtime/src/shared/newchain.ts`; **no** runtime import) |
| Wallet UI | `explorer/src/lib/injectedWallets.ts` + `Web3WalletBar`: list installed wallets → user picks → `eth_requestAccounts` on that provider only. Desktop discovery = EIP-6963; do **not** read `window.ethereum` before pick (Phantom auto-login). Mobile with no inject: deep-link panel to open this page in MetaMask / Base / OKX / TP |
| Submit | `POST {archiveUrl}/newchain/request` with `schema`, `labOnly`, `notProductionDepin`, `notL1Nft`, `classId`, `user`, `nonce`, `salt` |
| Submit result UI | After loading ends, **Submit** panel must show outcome in-panel: **success** → `requestId` HashCapsule + `chainNftId` (+ optional `valueHash` / duplicate); **failure** → Archive `error` / `reason` (e.g. `ERR_INVENTORY_FROZEN`) + HTTP status. Store in `submitResult` — **do not** clear via list/health `load()` (`setError(null)` must not wipe submit outcome) |
| Nonce / salt | **Internal only** — auto-generated (`Date.now` + random bytes32); **not shown**, not user-editable; rotate after each successful submit |
| Read | `GET /newchain/chains`; after Connect, UI shows **Your chains (owner)** = rows where `user` equals the wallet (case-insensitive); `/health` new-chain counts when reachable |
| Sign | Optional `personal_sign` ownership smoke before POST; Archive **does not** verify user signature |
| Storage | Web3 session address in memory only; **never** persist private keys |
| Labels | `not L1 NFT` / `labOnly`; predicted `chainNftId` must never be tip NFT `42` |
| Nginx | Public `dle.conet.network` **must** proxy Mode A: `GET /newchain/chains`, `GET /newchain/queue`, `POST /newchain/request` → lab upstream `:27101` (same pool as `/health` / `/liveness`). **Do not** proxy `/newchain/bft` (archive peer only). Without these locations, same-origin Submit gets nginx **405** HTML (`non-JSON response`) |
| Forbidden | Painting as L1 birth certificate, 30-day qualification, Mock auction, or production DePIN; importing runtime; exposing nonce/salt as editable form fields; single “Connect” that blindly uses default `window.ethereum`; public `/newchain/bft` |

Home pill: **Lab new-chain (Web3)**. Hook: `useWeb3Wallet`. Tests: `explorer/test/lab-newchain-wire.test.ts`, `explorer/test/injected-wallets.test.ts`.

## Other Explorer invariants

- **Public HTTPS vhost (2026-08-21 / 2026-08-28):** `https://dle.conet.network/health` connect-timeout (~22s) was **not** archive extraHealth. Live nginx on `70.35.205.77` had been left on HTTP bootstrap (`listen 80` only; no `:443`; `location /` `try_files` SPA). Public `:80` 200’d HTML; public `:443` timed out for `/`, `/health`, and `/liveness`. Certificate already existed (`/etc/letsencrypt/live/dle.conet.network/`, expiry 2026-11-12). Root cause: `scripts/deployDleExplorer.sh` always overwrote the live vhost with `nginx-dle.conet.network.http-bootstrap.conf` first; `--skip-tls` or a failed TLS step left that as the site. **Must:** if the Let’s Encrypt cert exists, install the full HTTPS + proxy conf and **never** leave bootstrap live. `--skip-tls` means skip certbot only. SPA `try_files` must not answer `/health` or `/liveness`.
- **nginx boot persist (2026-08-28):** On `70.35.205.77`, `nginx.service` **must** stay `systemctl enable`’d (`WantedBy=multi-user.target`). Drop-in `/etc/systemd/system/nginx.service.d/dle-persist.conf` (`Restart=always`, `RestartSec=2s`) so the process comes back after reboot or crash. `deployDleExplorer.sh` installs that drop-in and `enable --now`. Do **not** start nginx only from a one-shot SSH/`nginx` CLI. Do **not** restart geth / beacon-chain / validator to “keep Explorer up”.
- Public probes: prefer `GET /liveness` (process-up). Full `/health` is extraHealth + clusterView. Nginx `/health`: `proxy_read_timeout 12s`, `proxy_next_upstream_tries 1` (do not cascade 4×8s). Explorer `FETCH_MS` is **12s** to match. Tokyo fd-05 from `.77` can take ~7s; that is not a public connect-timeout.
...
- Cache only archive URL + last trusted snapshot. No wallet keys on disk. Session keys on `/mock-auction` and injected Web3 address on `/lab-newchain` are memory-only and cleared on reload.
