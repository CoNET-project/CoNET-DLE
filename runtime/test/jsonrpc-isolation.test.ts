...
test('facade never advertises CoNET L1 chain id 224422', () => {
  const chainId = dispatchArchiveJsonRpc(
  ...
  const poisoned = dispatchArchiveJsonRpc(
...
test('empty and oversized batches are invalid JSON-RPC requests', () => {
  const empty = dispatchArchiveJsonRpcEnvelope([], info)
  assert.equal(empty.ok, false)
  const huge = dispatchArchiveJsonRpcEnvelope(
