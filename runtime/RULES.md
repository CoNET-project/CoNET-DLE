...
`/health` may expose `tradeRpcCustodyConfigured` / `tradeRpcCustodyMode` (`rpc` | `hook` | `clientFlags`), `tradeOnChainSettleConfigured` / `tradeOnChainSettleMode`, and **Round 5** `tradeListConfigured` / `tradeListMode` (`rpc` | `hook`). **MVP round 2 (2026-08):** Archive-side custody via `shared/mockL1Custody.ts`; in-process demo `npm run mock-auction-demo`. **MVP round 3 (2026-08):** on-chain settle via `shared/mockL1Settle.ts`; E2E `npm run mock-auction-e2e` (needs `MOCK_L1_*`); root one-shot `npm run dle:mock-auction-e2e` (hardhat node → deploy → e2e). **MVP round 4 (2026-08):** Explorer may POST `/trade/settle` (authority key stays on Archive); Settlement summary shows phase + `settlementTxHash` capsules. **MVP round 5 (2026-08):** Explorer may POST `/trade/list` with session seller key; Settlement summary shows `listTxHash` before settle.
