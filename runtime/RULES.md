...
| `POST /trade/list` | **MVP round 5:** seller escrows NFT via `MockDleAuctionSettlement.list`. Body `{ candidateHash, sellerPrivateKey }` (lab session key; **not** stored). Must match sell `maker`. Hook `submitL1ListTx` or `MOCK_L1_RPC_URL` + `MOCK_L1_SETTLEMENT` → `listMockL1Auction`. Sets `listTxHash` / `listError` |
| `POST /trade/unlist` | **MVP round 9:** seller reclaim via `MockDleAuctionSettlement.unlist`. Body `{ candidateHash, sellerPrivateKey }` (lab session key; **not** stored). Requires `listTxHash`; phase `match_certified` **or** `settlement_failed`. Hook `submitL1UnlistTx` or RPC → `unlistMockL1Auction`. On success: set `unlistTxHash`, **clear `listTxHash`**, clear list errors; **do not** change phase. WAL `trade-unlist` |
| `POST /trade/approve` | **MVP round 6:** buyer ERC-20 `approve(settlement, amount)` for quote. Body `{ candidateHash, buyerPrivateKey, amount? }` (lab session key; **not** stored). Must match buy `maker`; default amount = `clearingPrice`. Hook `submitL1ApproveTx` or RPC → `approveMockL1AuctionQuote` (idempotent if allowance sufficient). Sets `approveTxHash` / `approveError` |
| `POST /trade/preflight` | **MVP round 8:** read-only settle readiness via `evaluateSettlePreflight` (phase + `listTxHash` + `approveTxHash` + optional RPC eth_call). Returns `checks` / `fees` / `onChain`. **Does not** mutate phase |
| `POST /trade/settle` | Mode A settlement states. **MVP round 3:** `executeOnChain: true` / `MOCK_L1_SETTLE_ONCHAIN` / `submitL1SettlementTx` hook → authority calls `MockDleAuctionSettlement.settle` (seller must have `list`ed first). Success → `settlement_submitted` → `settled` with real `settlementTxHash`; failure → `settlement_failed`. **MVP round 7/8:** unless `skipSettlePreflight: true`, refuse on-chain settle via shared `evaluateSettlePreflight` without record `listTxHash` + `approveTxHash` (400, phase stays `match_certified`); when RPC+settlement set, also `preflightMockL1AuctionSettle` eth_call gate before `executeOnChain`. **MVP round 9:** `outcome: 'failed'` marks `settlement_failed` for lab recovery (then unlist) |
...
`/health` may expose `tradeRpcCustodyConfigured` / `tradeRpcCustodyMode` (`rpc` | `hook` | `clientFlags`), `tradeOnChainSettleConfigured` / `tradeOnChainSettleMode`, **Round 5** `tradeListConfigured` / `tradeListMode` (`rpc` | `hook`), **Round 6** `tradeApproveConfigured` / `tradeApproveMode` (`rpc` | `hook`), and **Round 9** `tradeUnlistConfigured` / `tradeUnlistMode` (`rpc` | `hook`). **MVP round 2 (2026-08):** Archive-side custody via `shared/mockL1Custody.ts`; in-process demo `npm run mock-auction-demo`. **MVP round 3 (2026-08):** on-chain settle via `shared/mockL1Settle.ts`; E2E `npm run mock-auction-e2e` (needs `MOCK_L1_*`); root one-shot `npm run dle:mock-auction-e2e` (hardhat node → deploy → e2e). **MVP round 4 (2026-08):** Explorer may POST `/trade/settle` (authority key stays on Archive); Settlement summary shows phase + `settlementTxHash` capsules. **MVP round 5 (2026-08):** Explorer may POST `/trade/list` with session seller key; Settlement summary shows `listTxHash` before settle. **MVP round 6 (2026-08):** Explorer may POST `/trade/approve` with session buyer key; Settlement summary shows `approveTxHash`; e2e/demo prefer Archive list + approve after attest. **MVP round 7 (2026-08):** settle preflight (`listTxHash`/`approveTxHash` + optional RPC eth_call); CLI `list`/`approve` + settle `--executeOnChain`; Explorer one-shot **List → Approve → Settle**. **MVP round 8 (2026-08):** read-only `POST /trade/preflight` + shared `evaluateSettlePreflight`; CLI `preflight`; Explorer Preflight CTA + fee split on Settlement summary. **MVP round 9 (2026-08):** `POST /trade/unlist` + `MockDleAuctionSettlement.unlist`; lab `outcome: 'failed'` → `settlement_failed` then unlist reclaim; Explorer Unlist / Mark failed / Cancel sell; CLI `unlist`. **MVP round 10 (2026-08):** e2e/demo `MOCK_L1_E2E_MODE` / `MOCK_L1_DEMO_MODE` = `settle`|`recovery` (recovery = list → fail → unlist + `ownerOf` assert); one-shot `dle:mock-auction-e2e` runs recovery **then** settle on the same deploy. **MVP round 11 (2026-08):** CI gates — DLE `npm run mock-auction-ci` + parent Hardhat `test:dle` / `dle:mock-auction-e2e`; Explorer recovery English copy + **Mark failed → Unlist**.
...
| Step | Status | Deliver |
|---|---|---|
| **P8a** | landed | `inventoryFreeze.ts` + `indexLabHashObject` refuse **new** locators (`ERR_INVENTORY_FROZEN`). Catch-up `putLocator` is not frozen. `inventoryShouldFreeze()` = `hasUnseatedActive()` **or** local/roster `STATE_CHALLENGE`. **Do not** freeze on `pendingChallenge !== null` (`status()` would fake-freeze QUALIFIED). Heartbeat/`applyInventoryFreeze` must use `resolveInventoryFreezeState(auto, autoReason)` — operator sticky (`operator-inventory-freeze.json`, reason `operator`) outranks auto-unfreeze. `POST /sync/inventory-freeze` accepts only `{frozen:true}`; live HTTP must not unfreeze. `lab-cli` stops BFT / newchain / on-demand while frozen; `start()` resets `stopped`. Health: `bftProcessStarted` vs `bftDiskNetworked` (`networked: true` is the disk/facade bit, not `engine.start()`). Fresh `POST /newchain/request` → 409; duplicates stay 200. |
...
**MVP exclude:** `74.208.224.45`, `198.251.77.98`, and leftover `216.225.193.174`. Do **not** SSH, keep-deploy, `extraPeers`, on-demand hook, newchain-user, or nginx-upstream those IPs. `loadLabHosts` / `runSsh` / `runScp` must refuse them. `agentConfigFor(fd-01)` / `fd-03` / `fd-06` must succeed; peers must use `45.132.74.220`, `45.132.74.221`, and `70.35.205.77`. Historical `pilot/evidence/**` may still name the old IPs — do **not** rewrite evidence. This exclude is **not** an L1 health-check delist.
...
| `fd-06-ionos-174` | `70.35.205.77` | official standby (keep-data remapped 2026-08-18; DLE isolated in `~/dle-30d-lab`) |
...
**Explorer nginx** (`dle.conet.network`, 2026-08-17 authorized): `45.132.74.220:27101`, `45.132.74.221:27101`, `167.254.243.38:27101`, `170.205.39.67:27101`. Do **not** proxy shared-beacon `216.225.197.189` or standby `212.227.242.207`. Do **not** proxy leftover `216.225.193.174`. Do **not** add Explorer host `70.35.205.77:27101` to upstream (fd-06 standby is now colocated there; standby is not an upstream). Do **not** stop nginx / `dle-ondemand-clients` / `dle-newchain-user` on `.77` when starting the fd-06 DLE.
...
**P25 Explorer overlays (2026-08-17, historical):** Explorer Certificates + Home show **non-green** read-only chips for `officialStandbysReady` / `hashIndexCommittedInAc` (`explorer/src/lib/labOverlays.ts`). Green seating pills stay `seatingQualified === true` only (`archiveSeating.ts` unchanged). Missing overlay fields omit the chip (trusted-fetch). Tests at P25: `npm run explorer:test` **8/8**. **P25 public SPA (2026-08-18T00:18:49Z, historical):** `index-U1o9ul_I.js` replaced stale `index-DaEv6psZ.js`. Live `/health` at that publish still `officialStandbysReady=false` (`count=1`), `hashIndexCommittedInAc=false`, `seatingQualified=true`, `pilotStartedAt=null` — chips were warn/neutral, **not** green ready. **P25 seating-copy honesty (same gate, not P26):** Home / archive-detail seating copy is lab EIP-712, never HMAC. Completing P25 **MUST NOT** start `pilotStartedAt`. **Current public SPA** is the later clock overlay `index-C8IdTq4H.js` (2026-08-18T10:15:00Z; `explorer:test` **10/10**). **Not** 7/7 healthy / **not** 30-day / **not** production.

**fd-06 remapped (2026-08-18, not P26):** Operator authorized `70.35.205.77` for the official standby seat `fd-06-ionos-174` (`domainId` + wallet stay). Keep-data copied from leftover `216.225.193.174`; leftover IP is MVP-excluded. DLE lives only in `~/dle-30d-lab` on TCP **27101**. This host already runs Explorer nginx, `dle-ondemand-clients`, and `dle-newchain-user` — do **not** stop those, and do **not** add `.77:27101` to Explorer nginx upstream. Use `npm run lab:keep-remap-fd06` (keep-data, `onlyDomainIds` fd-06 + P11 extras). Do **not** run `lab:deploy-g1-keep` / `lab:keep-remap-l2` just to refresh fd-06. Do **not** restart leftover L1. Do **not** count `fd-08`. Completing this remap **MUST NOT** start `pilotStartedAt`. **Scrape 2026-08-18T00:52Z:** honest **7/7** `GET :27101/liveness` `LIVE_OK` (fd-06 on `70.35.205.77`; leftover `.174` timeout). fd-01 `/health` still `officialStandbysReady=false` (`count=1`; ready is **not** sticky). Four roots match fd-06 (`leafCount=9141`). **Fan-out 2026-08-18T00:57Z:** keep-data left fd-06/fd-07 with a locally matching `ArchiveStandbyReadiness`; `gossipOwnStandbyReady` early-returned and did not POST. Forwarding the current lab EIP-712 envelopes to official `:27101/sync/standby-ready` returned 200 on all 7. At `00:57:49Z` six official seats were `officialStandbysReady=true` (`count=2`; fd-06 still `count=1`). Unfrozen inventory then grew (`leafCount` 9141→9145); at `00:59:38Z` all seven were `ready=false` again. `pilotStartedAt=null`. Engine now reuses a matching local envelope but **still fan-outs** every QUALIFIED tick (test in `runtime/test/sync-standby-ready.test.ts`). Live hosts still run the old early-return until the next authorized standby keep — do **not** run `lab:deploy-g1-keep` just to pick this up. **Peer-stale 2026-08-18T01:03Z:** fd-01 / fd-07 `config.json` still pointed fd-06 at leftover `.174`. **Peer-refresh 2026-08-18T01:10Z:** `npm run lab:keep-refresh-fd06-peers` ok (`STOP_LAB_ONLY`; no leftover EL/CL). Official six + fd-06 keep-restarted; every official `config.json` now uses `http://70.35.205.77:27101` for fd-06 (`.174` count=0). Honest **7/7** LIVE_OK at 01:11Z (4/7 `ready=true` `count=2`). At 01:12:01Z seven seats flickered `ready=true` `count=2` (fd-05 `/liveness` timed out that pass; `/sync/status` 200). At 01:12:24Z honest **7/7** LIVE_OK again; only fd-03 + fd-06 still `ready=true` (roots already drifted `0x57a38d…` → `0x77ae8a…`). `.77` coexistence held (nginx 9; `:80`/`:443`/`:27101`; lab pid 208858). `pilotStartedAt=null`. Ready is **not** sticky while inventory is unfrozen. 7/7 liveness + a flicker of `ready=true` is **not** the 30-day gate. **Split 2026-08-18T07:05Z / 07:08Z:** honest **7/7** LIVE_OK; every official seat `officialStandbysReady=false` (`count` 0 or 1). The two official standbys sit on **different four roots** (camp A fd-01/03/06 leaf 9640→9645; camp B fd-02/04/05/07 leaf 9641→9646). QUALIFIED catch-up (30s) cannot close a +1 leaf race while unfrozen cataloguing / lab BFT still grow. Envelope POST cannot make `count=2`. **Flicker 2026-08-18T08:01:32Z:** 6/7 `ready=true` count=2 with both official standbys on leaf 9712; **21s later** fd-07 already on 9713 and ready dropped to 2/7. Do **not** keep-restart to chase ready. **Operator authorized inventory freeze (2026-08-18, not P26):** `npm run lab:keep-freeze-inventory` — `STOP_LAB_ONLY` official 7 + extra `fd-08` first, keep-data start with `~/dle-30d-lab/data/operator-inventory-freeze.json`, then `POST /sync/inventory-freeze` `{frozen:true}` only. Engine + tests: `runtime:test` **157/157**; `pilot` **14/14**; `explorer:test` **8/8**. **Live freeze 2026-08-18T08:35Z:** `lab:keep-freeze-inventory` ok. Official 7 + extra fd-08 `inventoryFrozen=true` `reason=operator`. Honest **7/7** LIVE_OK; `officialStandbysReady=true` count=2; leaf **9750** four roots aligned (`lastACRef` `0xbc4d0655…`); `bftProcessStarted=false`; `pilotStartedAt=null`. Confirm scrape ~08:36Z same (fd-04 `/sync/status` filled after the first pass). Evidence: `pilot/evidence/conet-dle-p23-live-2026-08/operator-inventory-freeze.json` (do **not** rewrite `fd06-remap-70-35-205-77.json`). Ready + freeze is **not** the 30-day clock. **Operator authorized clock start (2026-08-18, not P26, not qualification):** `npm run lab:start-pilot-clock` — keep-data; preserve freeze; `POST /sync/pilot-clock` `{start:true, warmupStartedAt, pilotStartedAt}`. Persist `~/dle-30d-lab/data/operator-pilot-clock.json`. `/health` may show `warmupStartedAt` / `pilotStartedAt` / `clockIsNotQualification`. `pilotQualified` stays false. Counters stay 0. Explorer nginx must **not** expose that POST. Green pills stay `seatingQualified === true` only. Do **not** invent P26. **Live clock 2026-08-18T09:53:58.092Z:** official 7 + extra fd-08 `warmupStartedAt=2026-08-14T17:10:16.786Z` `pilotStartedAt=2026-08-18T09:53:58.092Z` `warmupComplete=true` `pilotRunning=true` `pilotQualified=false` `clockIsNotQualification=true`; honest **7/7** LIVE_OK; freeze kept; `officialStandbysReady=true` count=2; leaf **9750**; counters still 0. Evidence: `pilot/evidence/conet-dle-p23-live-2026-08/operator-pilot-clock.json` + `pilot/evidence/conet-dle-30d-lab-2026-08/gate.json`. Engine + tests: `runtime:test` **159/159**; `pilot` **19/19**; `explorer:test` **10/10**. Clock is **not** 30-day qualification. **Explorer clock overlay (2026-08-18, not P26):** Certificates + Home show a **non-green** `pilotClockPill` from `/health.pilotRunning` (else derive from `pilotStartedAt`). Running → warn `30-day clock running (not qualified)`; explicit not-started → neutral `30-day clock not started`; missing fields omit the chip. `pilotQualified: true` is ignored. Never `tone: 'ok'`. Green seating pills stay `seatingQualified === true` only. Tests: `npm run explorer:test` **10/10**. **Public SPA published (2026-08-18T10:15:00Z):** `https://dle.conet.network/` now serves `index-C8IdTq4H.js` (replaced pre-clock `index-U1o9ul_I.js`). Live `/health`: `pilotRunning=true`, `pilotStartedAt=2026-08-18T09:53:58.092Z`, `pilotQualified=false`, `clockIsNotQualification=true`, `officialStandbysReady=true` count=2. Clock chip is warn, **not** green qualified. Explorer-host `POST /sync/pilot-clock` returns 405 (not proxied). Do **not** invent P26.

**Runtime scrape 2026-08-20T07:43:59Z (live; historical):** after inventory freeze (`2026-08-18T08:35Z`) and pilot clock start (`pilotStartedAt=2026-08-18T09:53:58.092Z`), formal 7 still sticky frozen + clock-aligned. Snapshot: HTTP **7/7**, `pilotRunning` **7/7**, seating QUALIFIED **7/7**, `pilotQualified` **0/7**, `lastQuorumOk` **6/7**. Evidence: `pilot/evidence/conet-dle-p23-live-2026-08/runtime-review-2026-08-20T074359Z.json`. Canvas: `src/canvas/dle-mvp-runtime-review-2026-08-20.md`. Do **not** invent P26.

**Runtime scrape 2026-09-01T10:35:23Z (live; mid-window):** ~**46.75%** of the 30-day wall clock. Operator inventory freeze still sticky **7/7**; clock aligned **7/7**; `pilotRunning` **7/7**; seating QUALIFIED **7/7**; `pilotQualified` still **0/7**. Formal `lastQuorumOk` true **2/7** only (`fd-01`, `fd-04`) — down from Aug 20 **6/7**; record as heartbeat reachability drift, **not** as seating loss or unfreeze. `fd-05` had `lastPeerOk=0`. `leafCount` / `officialStandbysReady` **omit** on all formal seats this scrape (omit ≠ emptied inventory). Lab BFT / AC hash-index commit still **0/7** true. Extra `fd-08` excluded from official 5+2. Public `/health` single-upstream OK does **not** equal fleet quorum mean. Evidence: `pilot/evidence/conet-dle-p23-live-2026-08/runtime-review-2026-09-01Tmidwindow.json`. Canvas mid-window assessment: `src/canvas/dle-30d-freeze-midwindow-assessment-2026-09-01.md`. Whitepaper Revision **2026-09-01**. Do **not** invent P26; do **not** claim 30-day qualification.

**Runtime scrape 2026-09-23T15:16:45.701Z (live; final post-window assessment):** the 30-day wall-clock target passed on `2026-09-17T09:53:58.092Z`, but the result is **partial success / not qualified**. Formal seven report HTTP, clock alignment, operator freeze, `leafCount=9750`, and seating `QUALIFIED` **7/7**; `pilotQualified` is **0/7**, formal `lastQuorumOk` is **3/7** (`fd-01`, `fd-04`, `fd-06`), and `fd-05` reports peer count **0**. The observed roster is **9–10 entries** and differs across nodes because extra `fd-08`, `fd-09`, and `fd-10` are present. `officialStandbyReadyCount=4` on all formal seats is a **classification drift**, not four authorized official standbys: `isOfficialStandbyRole` excludes only `fd-08`, so Seoul extras `fd-09` / `fd-10` can be counted. The operator inventory freeze blocks new hash-catalogue writes but does **not** freeze roster membership; replay/catch-up locator writes remain allowed. BFT process start, AC hash-index commit, and production `C_G` availability remain **0/7**. Evidence: `pilot/evidence/conet-dle-p23-live-2026-08/runtime-review-2026-09-23Tpostwindow.json`. Do not auto-unfreeze, auto-promote standby, start a production signer, claim production readiness, or invent P26.
...
| **P25** | **landed (Explorer overlay + tests + historical SPA).** Certificates + Home **non-green** read-only overlays for `officialStandbysReady` / `hashIndexCommittedInAc` (`explorer/src/lib/labOverlays.ts`; P25 `explorer:test` 8/8). Home / archive-detail seating copy is lab EIP-712, never HMAC. P25 public SPA was `index-U1o9ul_I.js` (2026-08-18T00:18:49Z). **Current** public SPA is clock overlay `index-C8IdTq4H.js` (2026-08-18T10:15:00Z; `explorer:test` 10/10). Green pills stay `seatingQualified === true` only. Missing overlay fields omit the chip. | Changing `archiveSeating.ts` seating logic; painting overlays as production AC / 30-day; flipping tree `committedInAc`; inventing P26 |
...
**Must not:** treat `pilotStartedAt` as 30-day qualification; reset warmup; invent P26; wipe keepers / `fd-05`; promote `fd-08` to an official 8th voter; live-inject a missing object; restart EL/CL.
