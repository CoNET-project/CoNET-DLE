...
- Deploy / accept from compiled pilot CLI: `cd pilot && npm run build && node dist/src/cli.js lab-deploy-archive` then `lab-accept-archive` (P1 AC) and `lab-accept-ondemand` (P3 SelectionLog). Do not use root `tsx pilot/src/cli.ts` (inventory path is wrong).
