...
**Author:** Peter Xie  
**First draft:** 2023  
**Revision:** 2026-09-01 (30-day freeze mid-window ~46.75% wall clock: operator inventory freeze sticky **7/7**; clock aligned / seating `QUALIFIED` **7/7**; `pilotQualified` **0/7**; official `lastQuorumOk` degraded to **2/7** vs Aug 20 **6/7** — reachability only, not seating revoke or unfreeze; leaf/ready omitted (omit ≠ empty inventory); wall end ~`2026-09-17T09:53:58.092Z`; evidence `pilot/evidence/conet-dle-p23-live-2026-08/runtime-review-2026-09-01Tmidwindow.json`; canvas `src/canvas/dle-30d-freeze-midwindow-assessment-2026-09-01.md`; prior 2026-08-18 lab MVP review: control plane P0–P11 live; honesty track engine/tests 159/159; P23 keep-deploy 6/7 LIVE_OK + fd-01 409→accept; later fd-06 remap / inventory freeze / clock start are not P26; P24 isolated `node.ts` new-chain accept uses the same `officialStandbysReady` callback as `lab-cli` (no seating tick / no inventory freeze); not 7/7 healthy / not 30-day / not production; P25 Explorer overlays landed historically (Certificates + Home non-green chips; green seating still seatingQualified === true); 30-day clock started 2026-08-18T09:53:58.092Z (`pilotQualified=false`; clock ≠ qualification); Explorer clock overlay public SPA `index-C8IdTq4H.js` at 2026-08-18T10:15:00Z (explorer:test 10/10; replaced pre-clock `index-U1o9ul_I.js`); next 30-day wait / review; do not invent P26; lab P22: official standby readiness is lab EIP-712 `ArchiveStandbyReadiness`; extra `fd-08` does not count; new-chain accept waits for two official standbys — not production OperatorDomain / secp256k1 / 30-day qualification; prior lab P21: lab BFT vote/QC/AC bind `hashIndexRoot`; tree `committedInAc` stays false; overlay when AC carries a non-zero bound root — not production AC commitment / not 30-day qualification; prior 2026-08-16: lab facade implements `ArchiveStateChallengeV1`; G1 fd-05/fd-07 zero-join `accept.json` `ok:true` at leaf 4956 with four roots aligned — still not 30-day qualification; prior same day: ArchiveSyncQualificationV1: SYNCING catch-up is not seating-eligible; claimed sync needs a group-wide random state challenge + \(Q_A=4/5\) peer vote — §5.2.0f; still not 30-day qualification; prior same day: lab G2 hosts emit `hop1.ownGroupId` / `liveGroupIds` as L1 register tx `0xf781f2c23fe3b3dac09dc3e1929016b0af200ee93978e916df64d750876d5153`; laboratory keccak is alias-only; still not 30-day qualification; prior same day: lab G2 L1 registerLiveGroup landed; user-visible G2 Group ID is that tx; still not 30-day qualification; prior same day: lab P6 live keep: 7/7 per-chain AC, NFT 42 tip stayed `0x1`; new-chain genesis is laboratory \(Q_V=5/7\) HMAC + that chain's own \(Q_A=4/5\) PrevoteQC/AC — never NFT 42 Tendermint / `bft-state.json`; old `DleLabGenesisCertificateV1` stub is not an AC; still not L1 birth certificate / not 30-day qualification; prior same day: lab M7: tipStateRoot + membershipRoot are first-class HashObjectKind — Option C; not AC field aliases; still not L1 register / not 30-day qualification; prior same day: lab M6 second archive group + plane-wide gather live; G2 Group ID is a lab hash not an L1 register tx; plane-wide JSON-RPC `null` only after every live group returns trusted this-group `notFound`; one-group timeout is `unavailable`; still not production DePIN / not 30-day qualification; prior 2026-08-15: hash RPC fact-check: every DLE-committed hash returns `hit` or this-group `notFound`; `unavailable` only when the check cannot complete; PrevoteQC is first-class `kind=prevoteQc` — Option C; future hash kinds MUST add a typed `HashObjectKind`, not AC field aliases — §5.2.0e; lab M0–M5 hash pipe live after wipe; hop-1 `historyProviders` on lab HTTP :27101; per-group `hashIndexRoot` is an independent lab checkpoint, not an AC vote field — still not production DePIN; hash-only RPC must first hit `chainNftId`, then hop-1 proxy — §5.2.0e; in-group geth-style hot KV + freezer keyed by `(nft, height)`; per-group `HashIndexTreeV1` / `hashIndexRoot`; plane-wide `null` only after trusted not-found; prior same-day: on-demand wait-hook intra-group fan-out: hooks **MUST NOT** be treated as gossiped among archives; the miner/daemon **MUST** deliver the same wait-hook to every live archive in that group — §5.4 / §8.1; laboratory HTTP `POST /ondemand/hook` is a lab/MVP control-plane path, **not** production DePIN gossip and **not** explorer HTTPS; laboratory beacon ≠ L1 CL RANDAO — §7.8.5; HTTP 30-miner queue is measured lab evidence, **not** 30-day qualification — §15; prior 2026-08-14: global DLE RPC truth surface: any live archive is a valid RPC entry; foreign-group queries **MUST proxy** to that group’s `historyProviders` — §5.2.0d; EIP-155 plane Chain ID (testnet `0x44c45`) vs user-visible Group ID = L1 register tx hash; L1 uint `groupId` remains a storage key; Global Archive Routing Registry: archive wallets + hosted chain NFT ids, chain routing / history providers — §5.2.0d; 5-active + 2-standby archive groups + strict 4/5 quorum + executable Archive Tendermint v2 corpus/Archive A MVP + line-level Tendermint/WAL rules + AdaptiveRotationV1 + OperatorDomainRegistryV1 + L1QueueAccumulatorV1 scale profile + deterministic `dle.rs.v1` correct-encoding proof + challenged force exit + Treasury V3 canonical ERC-20 burn/remint gateway + **measured P1 economic evidence boundary: 100-USDC safety cap frozen, 10-USDC floor and 1.2× coverage still provisional** + L1-anchored seller orders + x402 v2 AI pay-per-use / API-gateway target profile + three-ledger protocol/execution/availability economics + Treasury-canonical L1 pool/TWAP asset admission; archives have no block-production right)
...
- **EIP-155 Chain ID vs Group ID:** the plane’s unique **EIP-155 Chain ID** (`eth_chainId` / wallets) distinguishes this DLE plane from CoNET L1 `224422` and other EVM chains. **CoNET-DLE Testnet** is `0x44c45` / `281669`. User-visible **Group ID** is that archive group’s L1 **register transaction hash**. L1 `archiveGroupId[tokenId]` remains a uint **storage key** (bootstrap `1`). Bootstrap is **one** live group; fission yields a second unique group (new register hash), then a third, and so on. CoNET L1 **Global Archive Routing Registry** records every participating archive **wallet address** and every hosted chain **NFT id**, so clients obtain **chain routing** and **which archives may serve that tip’s history** from L1 (§5.2.0d).
- **Global RPC truth + foreign-group proxy:** every live archive **MUST** expose the same global DLE RPC surface, so a client MAY query **any** archive as a single entry. If the request is **not** for that node’s own **Group ID** (facade `route(chainNftId)` = register hash), the node **MUST proxy** to `historyProviders(chainNftId)` — the hosting group’s archive wallets — and **MUST NOT** answer foreign-group state from a local replica as RPC truth. Foreign-group comparison is on **Group ID**, not EIP-155 (§5.2.0d).
- **Hash-only lookup hits a tip first:** if the client holds only a block / tx / AC / PrevoteQC / `daRoot` / `tipStateRoot` / `membershipRoot` hash, the entry **MUST** resolve a `HashLocatorV1` that includes **`chainNftId`**, then `route(nftId)` and hop-1 proxy. RPC **MUST** fact-check every hash that appears in this DLE group: `hit` (typed object by `kind`) or this-group `notFound`. `unavailable` is only for an incomplete check. A local miss is **not** plane-wide `null`. Own-group hot path is a geth-style KV `hash → (chainNftId, kind, height)` plus freezer keyed by `(nft, height)` with per-`kind` objects; each group commits `hashIndexRoot` for inclusion / non-inclusion proofs. Success without `chainNftId` is a protocol error (§5.2.0e).
- **Archive sync qualification:** a newly started or catching-up archive is `SYNCING` and **MUST NOT** receive seating eligibility. A self-reported completed sync is only `CLAIMED_SYNC`. Other current-active archives of the target group MUST issue a deterministic random state challenge covering **every hosted chain** of that group; seating requires an `ArchiveSyncQualificationCertificate` at \(Q_A=4/5\) (already \(>2/3\); **3/5 is forbidden**). This is **not** the 30-day pilot gate (§5.2.0f).
...
| **EIP-155 Chain ID** | **Plane-unique uint** | Wallets / `eth_chainId` / `net_version` use this to distinguish the DLE plane from CoNET L1 `224422` and other EVM chains. **CoNET-DLE Testnet** = `0x44c45` / `281669`. It is **not** Group ID and **not** the tip NFT id. See §5.2.0d. |
| **Group ID** | **L1 register tx hash** | User-visible identity of one 5-active + 2-standby archive roster. L1 `archiveGroupId[tokenId]` is only the uint **storage key** (bootstrap `1`). See §5.2.0d. |
...
| \(U_e\) | Number of **IdentityEligible** archives in **UnassignedPool** (bonded, activated, cooldown-complete, `OperatorDomainRegistryV1 = ELIGIBLE`), not present in any live consensus roster. \(U_e\) is **not** seating: vote / standby-ready / formation-accept still require `SyncQualified` for the target group (§5.2.0f) |
...
All five active archives and both dedicated standbys MUST complete bond, activation, cooldown, history synchronization, and readiness challenges before the group becomes live or accepts a new chain. The seating readiness proof is `ArchiveSyncQualificationV1` (§5.2.0f): catch-up `SYNCING` is ineligible; a claimed sync is not evidence; peers MUST randomly test every hosted chain of the target group (or the witness `historySnapshotRoot` / empty-inventory commitment when \(C_G\) is empty); the vote is \(Q_A=4/5\), not 3/5. The active acceptance quorum is \(Q_A=4/5\); both standby readiness signatures are mandatory because standby availability is an exit/liveness precondition, not consensus authority. Existing groups provide continuity through checkpoint/history commitments; they do not copy permanent active or standby members into the new group.
...
#### 5.2.0d EIP-155 Chain ID vs Group ID & L1 Global routing registry (product freeze)

**Identifier freeze.** CoNET-DLE has two user-visible identifiers. **EIP-155 Chain ID** is the unique uint for this DLE plane. Wallets, `eth_chainId`, and `net_version` **MUST** use it to distinguish this plane from CoNET L1 `224422` and other EVM chains. **CoNET-DLE Testnet** is `0x44c45` / `281669`. **Group ID** is that archive group’s L1 **register transaction hash**. Clients, explorers, RPC routers, and certificates **MUST** use the register hash as Group ID. L1 `archiveGroupId[tokenId]` / `nextGroupId` remain uint **storage keys** (bootstrap `1`) and **MUST NOT** be shown as Group ID.

| Identifier | Meaning | Must **not** be used as |
| --- | --- | --- |
| **EIP-155 Chain ID** | Unique uint for this DLE plane. Testnet = `0x44c45` / `281669` | Group membership or tip identity |
| **Group ID** | L1 register tx hash of one live 5-active + 2-standby roster | EIP-155 chain id or tip NFT id |
| **L1 uint `groupId`** | Registry storage key (`nextGroupId`, `archiveGroupId[tokenId]`). Bootstrap = `1` | User-visible Group ID or EIP-155 |
| CoNET L1 EVM `chainId` | Settlement L1 `224422` / `0x36ca6` | DLE plane id, Group ID, tip routing, AC `archiveShardId` |
...
**Bootstrap and fission.** The archive plane starts with **exactly one** live group (\(G_e=1\)); L1 `nextGroupId` issues the first unused uint and **never reuses** a dissolved number. The user-visible **Group ID** of that group is its L1 register tx hash. When \(U_e\ge7\), fission registers a **second unique group** with a **new** register tx hash, then a third, and so on. Dissolved hashes and uints are never reused. Existing groups keep currently bound tips; a new group receives only post-formation assignments unless a **MigrationCertificate** re-homes a tip (§5.2).
...
```text
GlobalGroupRecord = {
  groupId,                          // L1 uint storage key (bootstrap 1); not user-visible Group ID
  registerTxHash,                   // user-visible Group ID = this group's L1 register tx
...
Required views (normative). Implementation MAY be one UUPS facade or the composition of `ArchiveGroupRegistryV1` (rosters / wallets) and `DLEChainRegistry1155V1` (NFT → uint `groupId`). Either shape **MUST** expose:

| View | Returns | Use |
| --- | --- | --- |
| `liveGroupIds()` | Live **Group ID hashes** on the protocol facade (L1 Solidity today still returns uint keys) | Enumerate groups |
| `archivesOf(groupId)` | Five active + two standby **EOA wallets** at the current membership epoch | Who votes; who is the dedicated history committee. Solidity takes the uint key; the facade accepts the register hash |
| `chainsOf(groupId)` | Hosted chain **NFT ids** | Which tips this **group** hosts |
| `route(chainNftId)` | **Group ID hash** on the protocol facade (L1 Solidity today still returns the uint key) | **Chain routing** — not EIP-155 |
| `historyProviders(chainNftId)` | `archivesOf(route(chainNftId))` | Which archives may serve **authoritative** history for that tip |

**Routing rule.** To locate a tip: Group ID = facade `route(nftId)`, then contact `historyProviders(nftId)`. Only wallets listed for that group at the relevant L1 membership epoch are **authoritative** history providers. A wallet **not** listed for that group **MUST NOT** be treated as the canonical host. Clients MAY still dial `historyProviders` directly; they MAY also dial **any** live archive and rely on the proxy rule below.
...
| --- | --- | --- |
| **Own group** | facade `route(chainNftId) = self.Group ID` (register hash), or the method is strictly group-local (this group’s WAL / AC chain / assigned QUEUED slice) | MAY answer from the local authoritative store. The response **MUST** still carry or allow verification of AC / `membershipRoot`. Client finality remains the AC, not HTTP 200. |
| **Foreign group** | facade `route(chainNftId) ≠ self.Group ID`, or the request names another Group ID | **MUST proxy** to one or more wallets in `archivesOf(targetGroupId)` = `historyProviders(chainNftId)`, then return the **host-group** response (including proof bundle). Comparison is on **Group ID**, not EIP-155. **MUST NOT** invent, synthesize, or present local replica bytes as RPC truth for that foreign group. |
...
| Re-home / dissolve | Move or drop the NFT id; **never** reuse the old uint or the old register hash as a new Group ID |

Do **not** write public explorer hostnames into Solidity constants. The Global registry stores **wallets, uint `groupId` keys, and NFT ids**. The user-visible Group ID is the group’s L1 register tx hash.
...
| **EIP-155 Chain ID** | Unique plane uint (`eth_chainId`). Testnet = `0x44c45` | **No.** Wallets / `eth_chainId` only. |
| **Group ID** | Hosting 5-active + 2-standby committee; user-visible id = L1 register tx hash. L1 uint is storage key only | **Derived.** Facade `route(chainNftId)` returns the hash. |
...
Each live group maintains one sparse / sorted Merkle tree whose root `hashIndexRoot` is committed by that group’s \(Q_A\) through AC / membership checkpoint. Laboratory M5 lands the tree as an **independent checkpoint** (`committedInAc: false`). Lab P21 binds the live/bound root into lab BFT vote / QC / AC typed data; tree views stay `committedInAc: false`. Health/Explorer may show overlay `hashIndexCommittedInAc` when the AC carries a non-zero bound root. This is **not** the production AC commitment formula.
...
| Not | The tree is **not** a DA / membership / tip-state **trie**. Those 32-byte roots **are** hot locate keys (lab M7: each has its own `HashObjectKind` + typed object). The tree is still **not** those tries and **not** an AC field alias. |
...
| --- | --- |
| AC / `valueHash` (`kind=ac`); PrevoteQC / `prevoteQCRef` (`kind=prevoteQc`); `tipStateRoot` (`kind=tipStateRoot`); `membershipRoot` (`kind=membershipRoot`); block; transaction; `daRoot` proof package | Copyright plaintext / IPFS fragment |
...
- JSON-RPC `null` / plane-wide trusted not-found **only** after every **live** group has returned a **trusted** `notFound` for the queried namespace (M6; laboratory gather live; G2 L1 `registerLiveGroup` landed; hosts emit that tx). A completed this-group KV miss **MUST** be `notFound`, not `unavailable` and not `null`.
...
- Aliasing `tipStateRoot` / `membershipRoot` onto the AC instead of **their** typed `HashObjectKind` (lab M7)
- Cataloguing a new-chain `valueHash` as `kind=ac` before that chain's PrecommitQC (lab P6)
- Writing new-chain votes into NFT 42 `/bft/message` or `bft-state.json`
...
#### 5.2.0f ArchiveSyncQualificationV1 — catch-up is not seating (product freeze)

**Problem.** A newly started or restarted archive that has not caught the group's latest hashes is in **catch-up sync**. Self-reporting `eth_syncing = false`, HTTP `/health` success, laboratory `lastQuorumOk`, or HMAC peer pings **MUST NOT** grant archive seating eligibility. A node that only stores a subset of hosted tips (for example only NFT 42) **MUST NOT** be treated as holding that group's state.

**Two layers (do not collapse):**

| Layer | Name | Grants |
| --- | --- | --- |
| Identity | `IdentityEligible` | Archive NFT + bond + activation + cooldown + `OperatorDomainRegistryV1 = ELIGIBLE` + role-domain exclusion. May sit in `UnassignedPool` and increment \(U_e\). **Does not** grant vote, standby-ready, formation-accept, or `lastQuorumOk` seating. |
| Sync seating | `SyncQualified` | `ArchiveSyncQualificationCertificate` for a **specific target group** \(G\). Required before the identity may vote, count as a ready standby, accept `MembershipFormationCertificate` / `MembershipUpdateCertificate`, or be advertised as a voting peer. |

The 30-day pilot gate (`PilotQualificationGate`, `pilotStartedAt`, rotations / re-homes / takeovers) is a **different** production-admission clock. Completing §5.2.0f **MUST NOT** start that clock.

**Hosted set.** For live group \(G\), let \(C_G\) be the sorted set of every `chainNftId` whose L1 `archiveGroupId` names \(G\), plus the group's current `lastAC` / `membershipRoot` / `hashIndexRoot` objects. A forming group with empty \(C_G\) still challenges the witness `historySnapshotRoot` / empty-inventory commitment. **Every** element of \(C_G\) MUST appear in the challenge sample set. Sampling only NFT 42, only new-chain tips, or a self-chosen subset is invalid.

**FSM (per identity, per target group):**

```text
BOOT / RESTART / SELECTED
  → SYNCING
  → CLAIMED_SYNC          // self-report only; still ineligible
  → STATE_CHALLENGE       // peer random tests
  → QUALIFIED | REJECTED
```

| State | Normative effect |
| --- | --- |
| `SYNCING` | Local tip / `lastAC` / freezer / `hashIndex` lags any chain in \(C_G\), or WAL recovery is incomplete. **MUST NOT** sign AC / Prevote / Precommit, **MUST NOT** enter seating, **MUST NOT** be counted as a ready standby. Quorum stays \(Q_A=4/5\) of the current `membershipRoot`; a syncing member does **not** lower quorum. |
| `CLAIMED_SYNC` | The candidate asserts catch-up is complete. This assertion is **not** evidence. Other current-active archives **MUST** run `ArchiveStateChallengeV1`. |
| `STATE_CHALLENGE` | Deterministic random samples covering **all** of \(C_G\). Candidate answers with **local-freezer** bytes (`dle_getObject` semantics). Hop-1 / `historyProviders` / foreign proxy during the challenge is a fail. Challengers compare to their own trusted freezer. Timeout / mismatch / missing chain / remote parrot → reject. |
| `QUALIFIED` | `ArchiveSyncQualificationCertificate` with \(\ge Q_A=4/5\) current-active ACCEPT signatures. Seating may proceed. |
| `REJECTED` | Remain ineligible; return to `SYNCING`. Retry only after a new challenge nonce. |

**Who votes.** Voters are the **current active** `membershipRoot` of the **target group** \(G\). If \(G\) is forming, the L1-designated **witness group**'s current actives vote. The candidate **MUST NOT** vote on itself. Standbys, unassigned identities, and foreign-group archives **MUST NOT** vote. Plane-wide \(Q_G\) and “2/3 of whoever is online” are **rejected**.

**Threshold.** The informal phrase “2/3 of the other archives” maps to the already-frozen \(Q_A=\lfloor 2N_A/3\rfloor+1=4\) for \(N_A=5\), which is already \(>2/3\). Two 4-of-5 quorums intersect in at least three members. **3/5 is unsafe and forbidden** (two such quorums may intersect only in the one Byzantine member). Do not invent a second 2/3 counter over the six non-candidates or over all seven seats.

**Challenge seed (production):**
...
Freeze `hostedChainSetRoot`, `lastACRef`, and the candidate set **before** the bound beacon is known. Laboratory implementations **MUST** label a post-freeze keccak beacon as non-production and **MUST NOT** read `publicrpc` / `rpc1` as if it were live CL RANDAO.

Each sample is a challenger-known `(chainNftId, kind, hash|height)` already in honest freezers. **Every** chain in \(C_G\) MUST appear. “At least one sample per chain” is only the **coverage** constraint — it is **not** by itself the possession proof. For each hosted chain the sample set MUST be **stratified**: one tip-class object (`ac` or `tipStateRoot`); one non-tip historical object when that chain’s freezer height span \(> 0\); one `daRootProof` when the chain has DA; and the challenge as a whole MUST include random `hashIndex` leaves under the same `hashIndexRoot` (not tips only). The candidate MUST return the exact object bytes; a locator-only or “I have the tip hash” reply is a fail. Challenge answers MUST be local-freezer (`dle_getObject`). Using `dle_getByHash` hop-1 / `historyProviders` / any remote parrot during `STATE_CHALLENGE` is `REJECTED`.

**Soundness — why a pass means the candidate holds the group’s information (product freeze).**

Passing `ArchiveStateChallengeV1` asserts **seating-grade possession of group \(G\)’s committed inventory**. It does **not** mean a linear scan of every historical byte in one round, and it does **not** replace post-seat `UnavailableChallenge` / DA custody. The argument has three layers:

1. **Commitment binds the whole inventory.** The candidate MUST return byte-exact `lastAC`, `membershipRoot`, and `hashIndexRoot` matching the challengers’ trusted copies. `hashIndexRoot` is the per-group directory of every object hash the group currently catalogues (§5.2.0e). A node that only stored NFT 42, or only current tips, **cannot** match a multi-chain / multi-height `hashIndexRoot`. Matching the roots is the claim “I am looking at the same set the honest majority committed.”

2. **Unpredictable opening proves the claim is not a 32-byte cheat.** `hostedChainSetRoot`, `lastACRef`, and the candidate set are frozen **before** the bound beacon is known. After reveal, the same seed derives `samples[]` for every current-active voter. The candidate cannot pre-compute a cheat sheet. Each body sample requires the exact freezer bytes; each index-leaf sample requires an inclusion proof under the **same** `hashIndexRoot`. If the candidate is missing fraction \(f\) of non-root objects, the probability of surviving \(k\) independent body openings is at most \((1-f)^k\).

3. **Stratified \(C_G\) coverage stops “I only have one chain / only tips.”** Every hosted `chainNftId` appears; tip + history + DA + index leaves are drawn. Sampling only NFT 42, only new-chain tips, or only tip-class kinds is invalid and **does not** satisfy “has all of this group’s information.”

**What a pass does not prove:** future availability after seating; that the candidate stays honest; every Reed–Solomon chunk of every height (sampled heights require a reconstructible local set; continuous DA custody is post-seat, §5.2.1). A SYNCING node that merely proxies honest peers **must fail** (layer: local-only).
...
} + ≥ QA=4/5 current-active signatures
  + candidate acceptance (QUALIFIED only)
```

This certificate is an archive seating certificate under the unified \(Q_A=4/5\) rule (§5.2.1). It is **not** an Archive Certificate over a tip, **not** a PlacementCertificate, and **not** 30-day qualification.

**Applies to:** cold start / restart of an already-assigned active or standby; standby replacement drawn from `UnassignedPool`; the seven identities selected for a new group (empty \(C_G\) still uses the witness snapshot). History replication or a temporary sync service still **does not** make a node a member (§5.2).

**Laboratory honesty.** Isolated-lab HTTP `:27101`, HMAC `lastQuorumOk`, and `GET /liveness` prove reachability only. The lab facade now implements `ArchiveStateChallengeV1` (HMAC votes + laboratory keccak beacon — **not** production EIP-712 / CL RANDAO). Explorer **MUST** show a green seating pill only when `seatingQualified === true`. `/health` / `lastQuorumOk` remain **not** seating. Zero-join accept evidence: `pilot/evidence/conet-dle-sync-join-2026-08/`. This section does **not** start `pilotStartedAt`.

After the 2026-08-15 laboratory wipe, the lab facade implements **M0–M5** plus the **Option C** PrevoteQC object: own-group hash KV, freezer keyed by `(chainNftId, height)` with per-`kind` slots, `dle_locateHash` / `dle_getByHash` / `eth_*ByHash` on the same pipe, hop-1 object fetch to `historyProviders(chainNftId)` over laboratory HTTP `:27101`, and a per-group sorted Keccak `HashIndexTreeV1` whose `hashIndexRoot` is an **independent lab checkpoint** (`committedInAc: false` on tree views). Lab P21 binds the live/bound root into lab BFT vote / QC / AC; overlay `hashIndexCommittedInAc` when the AC carries a non-zero bound root — **not** the production AC commitment formula. A hit **MUST** include `chainNftId` and return the object for that `kind` (`ac` vs `prevoteQc` are different objects). A completed this-group miss returns `notFound` (`planeWideNull: false`), not plane-wide `null` and not `unavailable`. `dle_proveHash` returns inclusion or sorted-range non-inclusion; one-group non-inclusion is **not** plane-wide `null` and **MUST NOT** replace the O(1) KV Get. Own-group hop failure **MAY** fall back to the local freezer and **MUST** mark `usedLocalFallback`. Foreign-group hop failure **MUST NOT** present a local replica as RPC truth. `dle_getObject` is local-freezer only and **MUST NOT** recurse into `dle_getByHash`. Laboratory HTTP / HMAC / lab beacon remain **not** production DePIN. **M6** (second laboratory group + cross-group evidence) is implemented on the lab facade (2026-08-16): `planeDirectory` + `locatePlane`; G2 Group ID `0xf781f2c23fe3b3dac09dc3e1929016b0af200ee93978e916df64d750876d5153` is the L1 `registerLiveGroup` tx; hosts emit that tx as `hop1.ownGroupId` / `liveGroupIds`; laboratory keccak `0x7b3b8eb959dcc0f75a309fcc16e7f840efe76dc27f2ef0d4eca8b8617f9b1a07` aliases it (the keccak itself is not an L1 tx); JSON-RPC `null` only when every live group returns trusted this-group `notFound`; one-group timeout is `unavailable`. **M7** (2026-08-16) catalogues `tipStateRoot` and `membershipRoot` as first-class kinds with typed objects (`DleLabTipStateRootV1` / `DleLabMembershipRootV1`); a hit returns that object, not the AC. Same-hash later heights are first-write-wins; `ZERO32` is not catalogued; side-index failure MUST NOT fail the parent AC. **P6** (2026-08-16) laboratory new-chain genesis: after Mode A replay, archives MUST form a deterministic 5-of-7 validator HMAC (`DleLabValidatorQuorumV1`, `hmacForgeable: true`, `notProductionSecp256k1: true`) and, on G1 with `enableBft`, a **per-`chainNftId`** 4-of-5 PrevoteQC → PrecommitQC (= AC) over `POST /newchain/bft` (`DleLabNewChainBftMessageV1`). The new-chain AC **MUST** use `labChainObjectLocator` with that `chainNftId` (never `42`). Accept **MUST NOT** catalogue `valueHash` as `kind=ac` before the real AC (freezer append-only). G2 (`enableBft: false`) does Mode A + \(Q_V\) only. New-chain AC **MUST NOT** update NFT 42 tip / `eth_blockNumber`. Clusters remain \(G_e\). This is **not** an L1 birth certificate and **not** 30-day qualification (§15.20).
...
**Unified archive-certificate threshold (product freeze).** `PrevoteQC`, `PrecommitQC / AC`, `PlacementCertificate`, `MembershipFormationCertificate` formation witnesses, `CandidateRejectCertificate`, old-roster approval of `MembershipUpdateCertificate`, both source/target sides of `MigrationCertificate`, and `ArchiveSyncQualificationCertificate` (§5.2.0f) all require **\(Q_A=4/5\)** distinct active signers under the applicable `membershipRoot`. Queue ordering is not an archive certificate: `L1QueueAccumulatorV1` supplies it without \(Q_G\). No separate 3/5, 5/5, dynamic-online quorum, or “last signer” privilege exists.
...
3. The ordered ready `standby[0]` is promoted at an AC/checkpoint boundary. A replacement standby is selected from `UnassignedPool` with fresh L1 randomness and MUST pass `OperatorDomainRegistryV1`, role-exclusion, history/state/DA synchronization, and `ArchiveSyncQualificationV1` (§5.2.0f) before assignment eligibility resumes.
...
| `STANDBY_SYNCING` | Promote `standby[0]` by default; it must sync current history/state/DA and `lastAC`, then prove readiness via `ArchiveSyncQualificationV1` (§5.2.0f). A self-reported tip hash is not the readiness proof. |
...
- [ ] EIP-155 Chain ID is the unique plane uint (testnet `0x44c45`); user-visible Group ID is the group’s L1 register tx hash; L1 uint `groupId` is storage key only. Global Archive Routing Registry stores archive **wallets** + hosted chain **NFT ids**; clients route history via facade `route(nftId)` → `historyProviders`—never CoNET L1 `224422`, never treat EIP-155 as Group ID, never `tokenId` hash (§5.2.0d).
...
- [ ] Hash-only lookup first hits **`chainNftId`** (`HashLocatorV1`); then `route(nftId)` and hop-1 to `historyProviders`. Success without `chainNftId` is a protocol error. RPC fact-checks every DLE-committed hash: `hit` (typed by `kind`, including `prevoteQc`, `tipStateRoot`, and `membershipRoot`) or this-group `notFound`. `unavailable` only when the check cannot complete. Local miss is not plane-wide `null` (§5.2.0e).
...
- [ ] Archive seating requires `ArchiveSyncQualificationV1`: `SYNCING` catch-up is ineligible; claimed sync is not evidence; peers randomly test **every** hosted chain of the target group; vote is \(Q_A=4/5\) (not 3/5). HTTP `/health` / `lastQuorumOk` / self-reported `eth_syncing` are not seating. This is **not** the 30-day pilot gate (§5.2.0f).
...
20. **Hash-only RPC pipe (lab M0–M7 + P6 + Option C):** §5.2.0e is product-frozen. After the 2026-08-15 DLE lab wipe, the laboratory facade implements **M0–M5** plus first-class `kind=prevoteQc`: completed this-group miss → `notFound` (`planeWideNull: false`), never plane-wide `null` and never `unavailable` after a finished KV search; `unavailable` only when the fact-check cannot complete; own-group file KV `hash → (chainNftId, kind, height)`; append-only freezer keyed by `(chainNftId, height)` with per-`kind` objects (`ac` vs `prevoteQc`); `dle_locateHash` / `dle_getByHash` / `eth_getBlockByHash` / `eth_getTransactionByHash` share the pipe; a hit **MUST** include `chainNftId` and return the typed object for that `kind`; after locate, hop-1 fetches the slot from `historyProviders` on lab HTTP `:27101` (`dle_getObject` is local-only); each group exposes `dle_getHashIndexRoot` / `dle_proveHash` over a sorted Keccak `HashIndexTreeV1` (tree views stay `committedInAc: false`; lab P21 binds the root into lab BFT vote/QC/AC; overlay when AC carries a non-zero bound root — not production AC commitment; not the hot Get; one-group non-inclusion ≠ plane-wide `null`). **M7** (2026-08-16) makes `tipStateRoot` / `membershipRoot` first-class `HashObjectKind`s with typed objects (not AC aliases; first-write-wins; `ZERO32` skipped). Future kinds **MUST** add a typed kind, not `boundField` aliases. Explore (`https://dle.conet.network`) can search a 32-byte hash and shows hop + proof receipts. **M6** laboratory fission (2026-08-16) is live on seven greenfield hosts (`/home/peter/dle-m6-g2`, TCP 27101, BFT/ondemand off): G2 Group ID is the L1 register tx `0xf781f2c23fe3b3dac09dc3e1929016b0af200ee93978e916df64d750876d5153`; hosts emit that tx as `hop1.ownGroupId` / `liveGroupIds`; laboratory keccak `0x7b3b8eb959dcc0f75a309fcc16e7f840efe76dc27f2ef0d4eca8b8617f9b1a07` aliases it — **not** uint `2`; `locatePlane` yields plane-wide JSON-RPC `null` only after every live group returns trusted this-group `notFound`; one-group timeout is `unavailable`. HMAC ≠ EIP-712; laboratory beacon ≠ L1 CL RANDAO; HTTP hook ≠ production DePIN. **P6** (2026-08-16) laboratory new-chain genesis is \(Q_V=5/7\) HMAC + per-chain \(Q_A=4/5\) AC (never NFT 42 Tendermint). Completing M0–M7+P6 does **not** start 30-day qualification.
21. **Archive sync qualification (product freeze; lab facade live):** §5.2.0f. Catch-up `SYNCING` is not seating-eligible. Claimed sync is only `CLAIMED_SYNC`. Other current-active archives MUST randomly test that the candidate holds **every hosted chain** of the target group; seating is an `ArchiveSyncQualificationCertificate` at \(Q_A=4/5\) (already \(>2/3\); **3/5 forbidden**). A pass means **seating-grade possession of the committed inventory**: matching `lastAC` / `membershipRoot` / `hashIndexRoot`, unpredictable local-freezer openings, stratified tip+history+DA+index samples. One sample per chain alone is coverage, not the proof. Hop-1 / proxy during the challenge is a fail. HTTP `/health`, `lastQuorumOk`, and self-reported `eth_syncing` are reachability only. Laboratory HMAC / keccak beacon are **not** production EIP-712 / CL RANDAO. Zero-join accept: wipe G1 `fd-05` + `fd-07` DLE datadir only, then `SYNCING` → `QUALIFIED`. This is **not** `PilotQualificationGate` / `pilotStartedAt`.

---


22. **30-day freeze mid-window (2026-09-01):** Operator inventory freeze (`reason=operator`, sticky since `2026-08-18T08:35Z`) and the 30-day pilot clock (`pilotStartedAt=2026-08-18T09:53:58.092Z`) both still hold on the formal seven seats at scrape `2026-09-01T10:35:23Z` (~46.75% wall clock; target end `2026-09-17T09:53:58.092Z`). Seating remains `QUALIFIED` / `seatingQualified` **7/7**. `pilotQualified` remains **false** on all formal seats — wall-clock progress ≠ PilotQualificationGate. Formal heartbeat `lastQuorumOk` true degraded to **2/7** (`fd-01`, `fd-04`) versus the 2026-08-20 scrape **6/7**; that is a **reachability observation only**, not seating revocation and not an unfreeze. Formal `leafCount` / `officialStandbysReady` were omitted this scrape (omit ≠ emptied inventory). This is **not** production DePIN gossip, **not** L1 CL RANDAO, and **not** 30-day qualification. Evidence: `src/conet-layer2/pilot/evidence/conet-dle-p23-live-2026-08/runtime-review-2026-09-01Tmidwindow.json`. Canvas: `src/canvas/dle-30d-freeze-midwindow-assessment-2026-09-01.md`.
...
| **\(G_e\) / \(N_e\) / \(U_e\)** | Registered live-group count / unique active voting archive count / **IdentityEligible** unassigned count. With disjoint five-voter rosters, \(N_e=5G_e\); one fully serviceable group may form iff \(U_e\ge7\). \(U_e\) is not seating; seating needs `SyncQualified` (§5.2.0f). |
| **ArchiveSyncQualificationV1** | Seating FSM: `SYNCING` → `CLAIMED_SYNC` → `STATE_CHALLENGE` → `QUALIFIED` \| `REJECTED`. Peers randomly test every hosted chain of the target group; vote is \(Q_A=4/5\). Not the 30-day pilot gate (§5.2.0f). |
...
| **EIP-155 Chain ID** | Unique uint for this DLE plane (`eth_chainId` / wallets). CoNET-DLE Testnet = `0x44c45` / `281669`. Not Group ID, not CoNET L1 `224422`, not tip NFT id (§5.2.0d). |
| **Group ID** | User-visible archive-group identity = that group’s L1 **register transaction hash**. L1 uint `groupId` is only the registry storage key (bootstrap `1`). Fission issues a new hash; dissolved hashes are never reused (§5.2.0d). |
...
| **HashLocatorV1** | Successful hash-only locate result: **`chainNftId` required**, plus `kind` (`ac` \| `prevoteQc` \| `tipStateRoot` \| `membershipRoot` \| `block` \| `tx` \| `daRootProof`), `height`, optional `acRef` / debug `groupId`. A 200 without nft is a protocol error. New kinds add a `HashObjectKind`, not AC aliases (§5.2.0e). |
