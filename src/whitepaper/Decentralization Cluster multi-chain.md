...
- JSON-RPC `null` / plane-wide trusted not-found **only** after every **live** group has returned a **trusted** `notFound` for the queried namespace (M6; laboratory gather live; L1 `registerLiveGroup` for the second group still pending). A completed this-group KV miss **MUST** be `notFound`, not `unavailable` and not `null`.
